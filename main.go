...
Options:
  ...
  -d, --debug               Debug logging [default: false].`
	...
	for update := range updates {
		...
		if strings.HasPrefix(update.Message.Text, "/q ") {
			// TODO add group support (/command@bot)
			// split := strings.Split(update.Message.Text, "/q@"+bot.Self.UserName)
			// if len(split) < 2 {
			...
			if val != nil {
				...
				for _, a := range val.Movies {
					...
					button := fmt.Sprintf("%s [%d] [%s]", a.Title, a.Year, a.Imdb)
		...
		} else if imdb.MatchString(strings.ToLower(update.Message.Text)) {
			log.Println("Found IMDB ID")
			// Find the IMDB ID matched in the received message
			imdbId := imdb.FindString(strings.ToLower(update.Message.Text))
			// Find the IMDB ID matched in the received message
			title, _ := regexp.Compile("(.*) \\[.*\\] \\[tt[0-9]{5,10}\\]")
